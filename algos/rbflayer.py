...
class RBFLayer(Layer):
    ...
    def __init__(self, output_dim, name='rbflayer',initializer=None, **kwargs):
    ...
    def build(self, input_shape):
        ...
        self.betas = self.get_sigma(self.centers)
    ...
    def get_sigma(self, data):
            ...
            for i, point in enumerate(data):
                # Calculate the distance to all other points in the array
                dists = np.linalg.norm(data - point, axis=1)

                # Add up the distances and store the result in the distances array
                distances[i] = np.mean(dists)

            # Return the final distances array
            return distances
